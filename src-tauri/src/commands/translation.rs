...
use crate::services::ai::{
    claude::ClaudeProvider, ollama::OllamaProvider, openai::OpenAIProvider,
    zhipu::ZhipuProvider, AIProvider, ProviderConfig, StreamEvent, TranslationRequest,
    TranslationResponse,
};
...
async fn get_enabled_providers(db: &sqlx::SqlitePool) -> Result<Vec<ProviderConfig>, String> {
    ...
    let rows: Vec<DbProviderConfig> = sqlx::query_as(
        "SELECT provider_name, api_key, model, base_url FROM provider_configs WHERE enabled = 1 ORDER BY provider_name"
    )
    .fetch_all(db)
    .await
...
fn can_detect_language(config: &ProviderConfig) -> bool {
...
fn select_detector_config(providers: &[ProviderConfig]) -> Result<&ProviderConfig, String> {
    providers
        .iter()
        .find(|config| config.provider_name == "zhipu" && can_detect_language(config))
        .or_else(|| providers.iter().find(|config| can_detect_language(config)))
        .or_else(|| providers.first())
        .ok_or_else(|| "无可用服务商".to_string())
}
...
async fn detect_and_plan(
    ...
    let detected_lang = match config.provider_name.as_str() {
        ...
        _ => Err(crate::services::ai::AIError::Other(format!(
            "未知服务商: {}",
...
pub async fn translate_multi(
    ...
    let detector_config = select_detector_config(&providers)?;
...
pub async fn translate_multi_stream_individual(
    ...
    let detector_config = select_detector_config(&selected_providers)?;
