                    ...
                    match type_ {
                        CGEventType::KeyDown => {
                            ...
                            // Cmd + C 双击检测
                                ...
                                // macOS: only Cmd+C is considered "copy" for the double-tap trigger
                                if guard.meta_down {
                                    let now = Instant::now();
        ...
        if let Err(e) = grab(move |event: Event| {
            ...
            match event.event_type {
                EventType::KeyPress(key) => {
                    if matches!(key, Key::ControlLeft | Key::ControlRight) {
                        ...
                        if guard.alt_down {
                            ...
                            // 立即重置Alt状态，因为窗口弹出后可能丢失KeyRelease事件
                        ...
                        // Windows: Ctrl + 双击C触发翻译
                        // 必须在Ctrl按下的状态下才检测双击
                        app_debug!("检测到C键按下，当前Ctrl状态: {}", guard.ctrl_down);

                        if !guard.ctrl_down {
                            // 如果没有Ctrl，清空状态并直接返回
                            app_debug!("没有Ctrl键，清空双击状态");
                            guard.last_c_press = None;
                        } else {
                            let now = Instant::now();
                            let is_double = guard.last_c_press.map_or(false, |prev| {
                            ...
                            if !is_double {
                                app_info!("检测到第一次 Ctrl+C");
                                guard.last_c_press = Some(now);
                            } else {
                                app_info!("检测到双击 Ctrl+C！触发翻译...");
                                guard.last_c_press = None;
                                // 立即重置Ctrl状态，因为窗口弹出后可能丢失KeyRelease事件
                                guard.ctrl_down = false;
                                app_debug!("强制重置Ctrl状态，防止窗口弹出后状态残留");
                                let app_clone = app_handle.clone();
                                tauri::async_runtime::spawn(async move {
                                    handle_double_copy(app_clone).await;
                                });
                ...
                EventType::KeyRelease(key) => {
                    if matches!(key, Key::ControlLeft | Key::ControlRight) {
                        app_debug!("Ctrl键释放，重置双击状态");
                        guard.ctrl_down = false;
                        guard.last_c_press = None; // Ctrl释放时重置状态
                    } else if matches!(key, Key::MetaLeft | Key::MetaRight) {
