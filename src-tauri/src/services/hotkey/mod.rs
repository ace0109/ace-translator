...
struct DoubleTapState {
    ...
    suppress_next_ctrl_c: bool, // Windows: ignore Ctrl+C generated by our simulated copy
}
...
pub fn start_listener(app: tauri::AppHandle) {
    ...
    std::thread::spawn(move || {
        ...
        if let Err(e) = grab(move |event: Event| {
            let mut simulate_copy = false;
            let mut trigger_translate = false;
            ...
            match event.event_type {
                EventType::KeyPress(key) => {
                    if matches!(key, Key::ControlLeft | Key::ControlRight) {
                        ...
                        // Ignore the C press produced by our simulated Ctrl+C.
                        if guard.suppress_next_ctrl_c && guard.ctrl_down {
                            guard.suppress_next_ctrl_c = false;
                            return Some(event);
                        }

                        // Windows: use Alt+C double-tap as the trigger. On the first Alt+C,
                        // simulate Ctrl+C to copy selected text into the clipboard.
                        if !guard.alt_down {
                            return Some(event);
                        }
                        ...
                        if !is_double {
                            app_debug!("检测到第一次 Alt+C，触发复制");
                            guard.last_c_press = Some(now);
                            guard.suppress_next_ctrl_c = true;
                            simulate_copy = true;
                        } else {
                            app_info!("检测到双击 Alt+C！触发悬浮翻译...");
                            guard.last_c_press = None;
                            trigger_translate = true;
                        }
                    }
                }
            ...
            if simulate_copy {
                tauri::async_runtime::spawn(async move {
                    if let Err(e) = clipboard::simulate_copy() {
                        app_error!("模拟复制失败: {}", e);
                    }
                });
            }
