...
- 浮窗：双击复制（Command/Ctrl+C 连按）自动弹出并跟随鼠标，支持取消翻译、结果复制、固定/失焦自动隐藏。
- 翻译后端：接入智谱 GLM-4-FlashX，多目标批量翻译，统一的请求/结果处理链路。
